...
// Admins OR employees with role='pm' in the employees table
export function requirePM(req, res, next) {
  const token   = req.cookies?.mds_session;
  const session = validateSession(token);
  if (!session) return res.status(401).json({ error: 'Unauthorized.' });
  const u = session.user;
  if (!u?.isAdmin && u?.role !== 'pm') return res.status(403).json({ error: 'Admin or Project Manager access required.' });
  req.user = u;
  next();
}
