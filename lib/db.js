...
try { db.exec('ALTER TABLE clients ADD COLUMN tierId TEXT'); } catch {}
try { db.exec('ALTER TABLE clients ADD COLUMN slaId TEXT'); } catch {}
...
// Current step in a sequential approval chain (0-based). Only meaningful when
// the Approval Workflow is configured with steps; single/auto modes ignore it.
try { db.exec('ALTER TABLE psa_timesheets ADD COLUMN approvalStep INTEGER DEFAULT 0'); } catch {}
try { db.exec('ALTER TABLE projects ADD COLUMN quotedHours REAL DEFAULT 0'); } catch {}
try { db.exec('ALTER TABLE projects ADD COLUMN costCenterId TEXT'); } catch {}
...
// Task lifecycle timestamps — set when a task enters In Progress / Done.
try { db.exec('ALTER TABLE tasks ADD COLUMN startedAt TEXT'); } catch {}
try { db.exec('ALTER TABLE tasks ADD COLUMN completedAt TEXT'); } catch {}

// ── Dynamic job roles ────────────────────────────────────────────────────────
// Custom, admin-definable roles (e.g. "Sr Solutions Architect"). The 4 original
// enum values are seeded as SYSTEM roles using their own strings as ids, so the
// existing employees.role values keep resolving with zero data migration.
try {
  ...
  const rSeed = db.prepare('INSERT OR IGNORE INTO roles (id,name,description,isManager,isSystem,permissions,sortOrder,createdAt,updatedAt) VALUES (?,?,?,?,?,?,?,?,?)');
  rSeed.run('admin',      'Administrator',   'Full system access',                          1, 1, '{}', 0, rNow, rNow);
  rSeed.run('pm',         'Project Manager', 'Owns and manages projects; approves timesheets', 1, 1, '{}', 1, rNow, rNow);
  rSeed.run('supervisor', 'Supervisor',      'Oversees teams and approvals',                1, 1, '{}', 2, rNow, rNow);
  rSeed.run('resource',   'Resource',        'Team member who logs time',                   0, 1, '{}', 3, rNow, rNow);
} catch {}

// Permission Sets — named, reusable access templates (Replicon-style). `pages`
// is a JSON array of granted page keys; `capabilities` is a JSON object of
// module.action → bool for granular actions. Applying a set writes its page
// grants onto the target users' permissions (which the app already enforces).
try {
  db.exec(`CREATE TABLE IF NOT EXISTS permission_sets (
    id           TEXT PRIMARY KEY,
    name         TEXT NOT NULL,
    type         TEXT,
    description  TEXT,
...
// Org-structure lists (managed entities assigned to employees).
try { db.exec(`CREATE TABLE IF NOT EXISTS departments (
  id TEXT PRIMARY KEY, name TEXT NOT NULL, code TEXT, description TEXT, createdAt TEXT NOT NULL
)`); } catch {}
try { db.exec(`CREATE TABLE IF NOT EXISTS locations (
  id TEXT PRIMARY KEY, name TEXT NOT NULL, code TEXT, description TEXT, createdAt TEXT NOT NULL
)`); } catch {}
try { db.exec(`CREATE TABLE IF NOT EXISTS employee_types (
  id TEXT PRIMARY KEY, name TEXT NOT NULL, code TEXT, description TEXT, createdAt TEXT NOT NULL
)`); } catch {}
try {
...
try { db.exec(`CREATE TABLE IF NOT EXISTS project_categories (
  id        TEXT PRIMARY KEY,
  name      TEXT NOT NULL UNIQUE,
...
try { db.exec(`CREATE TABLE IF NOT EXISTS task_categories (
  id        TEXT PRIMARY KEY,
  name      TEXT NOT NULL UNIQUE,
...
try { db.exec(`CREATE TABLE IF NOT EXISTS priority_levels (
  id        TEXT PRIMARY KEY,
  name      TEXT NOT NULL UNIQUE,
...
try { db.exec(`CREATE TABLE IF NOT EXISTS client_tiers (
  id          TEXT PRIMARY KEY,
  name        TEXT NOT NULL UNIQUE,
  level       INTEGER NOT NULL DEFAULT 0,
...
try { db.exec(`CREATE TABLE IF NOT EXISTS billing_rates (
  id            TEXT PRIMARY KEY,
  role          TEXT NOT NULL,
...
try { db.exec(`CREATE TABLE IF NOT EXISTS cost_centers (
  id          TEXT PRIMARY KEY,
  name        TEXT NOT NULL UNIQUE,
  code        TEXT UNIQUE,
  description TEXT,
...
try { db.exec(`CREATE TABLE IF NOT EXISTS api_keys (
  id         TEXT PRIMARY KEY,
  name       TEXT NOT NULL,
...
try { db.exec(`CREATE TABLE IF NOT EXISTS webhooks (
  id        TEXT PRIMARY KEY,
  name      TEXT NOT NULL,
  url       TEXT NOT NULL,
...
try { db.exec(`CREATE TABLE IF NOT EXISTS alert_rules (
  id        TEXT PRIMARY KEY,
  name      TEXT NOT NULL,
...
try { db.exec(`CREATE TABLE IF NOT EXISTS project_template_tasks (
  id             TEXT PRIMARY KEY,
  templateId     TEXT NOT NULL REFERENCES project_templates(id) ON DELETE CASCADE,
  name           TEXT NOT NULL,
  estimatedHours REAL DEFAULT 0,
  sortOrder      INTEGER DEFAULT 0
)`); } catch {}

try { db.exec(`CREATE TABLE IF NOT EXISTS sla_tiers (
  id              TEXT PRIMARY KEY,
  name            TEXT NOT NULL UNIQUE,
  priority        INTEGER NOT NULL DEFAULT 0,
  responseHours   REAL NOT NULL DEFAULT 24,
  resolutionHours REAL NOT NULL DEFAULT 72,
  color           TEXT NOT NULL DEFAULT '#60a5fa',
  createdAt       TEXT NOT NULL
)`); } catch {}

try { db.exec(`CREATE TABLE IF NOT EXISTS workflow_rules (
  id         TEXT PRIMARY KEY,
  name       TEXT NOT NULL,
...
try { db.exec(`CREATE TABLE IF NOT EXISTS support_tickets (
  ...
  createdAt   TEXT NOT NULL,
  updatedAt   TEXT NOT NULL
)`); } catch (e) { logger.warn({ err: e }, 'support_tickets table setup failed'); }
try { db.exec('CREATE INDEX IF NOT EXISTS idx_tickets_user   ON support_tickets(userId)'); } catch {}
try { db.exec('CREATE INDEX IF NOT EXISTS idx_tickets_status ON support_tickets(status)'); } catch {}
