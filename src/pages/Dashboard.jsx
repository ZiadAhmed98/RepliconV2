...
export default function Dashboard({ dataMatrix }) {
  // Local State for the Compliance Card
  ...
  const metrics = useMemo(() => {
    ...
    factTable.forEach(row => {
      ...
      // Build Time Trend Map (Burn vs Capacity)
      if (row.date > 0) {
    ...
    Object.keys(dimensionTable).forEach(pName => {
      ...
      // Upcoming Rolloffs Forecasting
      if (isActive) {
        let engs = new Set();
        factTable.forEach(r => { if(r.project === pName && r.date > (nowTs - 30*86400000) && r.act > 0) engs.add(r.user); });
        if(engs.size > 0) {
    ...
    // 3. Process Capacity for Trend Chart
    let trendLabels = [], trendActuals = [], trendCapacity = [];
    Object.keys(timeTrendMap).sort().forEach(k => {
      let [year, month] = k.split('-'); 
      let mStart = new Date(year, parseInt(month)-1, 1).getTime(); 
      let mEnd = new Date(year, parseInt(month), 0).getTime();
    ...
    return {
      kpis: {
        ...
        actual: Math.round(tAct), estimated: Math.round(tEst), quoted: Math.round(tQuoted),
      },
  ...
  // Formatter for large numbers (e.g., 15000 -> 15k)
    ...
    <div>
      ...
      <div className={styles.sectionHeader}>
        <div className={styles.titleArea}>
          ...
          <div className="badges-container"><span className="badge-base period-badge">All Time</span></div>
        </div>
      </div>
      ...
      <div className={styles.kpiGrid}>
        ...
        {/* COMPLIANCE CARD WITH ARROWS & SPARKLINE */}
        <div className={`kpi-card ${styles.complianceCard}`} onClick={() => setIsCompModalOpen(true)}>
          ...
          <div className={styles.sparklineContainer}>
            <Chart type="area" width="100%" height={35} series={[{ data: metrics.compliance.sparkline || [] }]}
              options={{ chart: { sparkline: { enabled: true } }, stroke: { curve: 'smooth', width: 2 }, colors: ['#f43f5e'],
      ...
      {/* --- RESTORED: PORTFOLIO BURN & TOP CARDS --- */}
      <div className={styles.chartRow}>
        <div className="chart-card">
          ...
          <div className={styles.chartWrapper}>
            <Chart type="area" width="100%" height={320}
              ...
              options={{ ...chartDefaults, chart: { stacked: true }, colors: ['#f59e0b', 'rgba(255,255,255,0.1)', '#3b82f6'],
                stroke: { curve: 'smooth', width: 2 }, fill: { type: 'solid', opacity: [0.1, 0.4, 0.8] },
                xaxis: { categories: metrics.deepEffort.labels, labels: { show: false }, axisBorder: { show: false }, axisTicks: { show: false } },
                yaxis: { labels: { formatter: (v) => fmtK(v), style: { colors: '#a1a1aa' } } },
        ...
        <div className="chart-card">
          ...
          <div className={styles.chartWrapper}>
            <Chart type="bar" width="100%" height={320} series={[{ name: 'Hours', data: metrics.topClients.map(c => Math.round(c.val)) }]}
              options={{ ...chartDefaults, colors: ['#10b981'], plotOptions: { bar: { horizontal: false, borderRadius: 4, distributed: true, columnWidth: '40%' } }, dataLabels: { enabled: false }, xaxis: { categories: metrics.topClients.map(c => c.name), labels: { style: { colors: '#a1a1aa' }, rotate: -45, trim: true } }, grid: { borderColor: '#27272a', strokeDashArray: 4 }, legend: { show: false } }} />
          </div>
        </div>
        <div className="chart-card">
          <h4><i className='bx bx-trophy' style={{ color: 'var(--accent-yellow)' }}></i> Top Employees</h4>
          <div className={styles.chartWrapper}>
            <ul className={styles.insightList}>{metrics.topEmployees.map((e, i) => <li key={i} className={styles.insightItem}><div className={styles.insightInfo}><div className={styles.insightRank}>{i + 1}</div><span className={styles.insightName}>{e.name}</span></div><span className={styles.insightVal}>{fmtInt(e.val)} hrs</span></li>)}</ul>
          </div>
        </div>
      </div>

      {/* --- RESTORED: TREND VS CAPACITY & BILLABLE --- */}
      <div className={styles.chartRowHalf}>
        <div className="chart-card">
          ...
          <div className={styles.chartWrapper}>
            <Chart type="line" width="100%" height={320}
              series={[ { name: 'Capacity', type: 'line', data: metrics.trend.cap }, { name: 'Actual Burn', type: 'area', data: metrics.trend.act } ]}
              options={{ ...chartDefaults, colors: ['#a1a1aa', '#3b82f6'], stroke: { curve: 'smooth', width: [3, 2] },
                fill: { type: ['solid', 'gradient'], gradient: { shadeIntensity: 1, opacityFrom: 0.4, opacityTo: 0.0, stops: [0, 100] } },
                xaxis: { categories: metrics.trend.labels, labels: { style: { colors: '#a1a1aa' } }, axisBorder: { show: false }, axisTicks: { show: false } },
        ...
        <div className="chart-card">
          ...
          <div className={styles.chartWrapper}>
            <Chart type="donut" width="100%" height={320} series={[metrics.billable, metrics.overhead]} options={{ ...chartDefaults, labels: ['Billable', 'Non-Billable'], colors: ['#10b981', 'rgba(255,255,255,0.1)'], stroke: { width: 0 }, plotOptions: { pie: { donut: { size: '75%' } } }, dataLabels: { enabled: false }, legend: { position: 'bottom', labels: { colors: '#a1a1aa' } } }} />
          </div>
        </div>
      </div>

      {/* --- RESTORED: AT-RISK & OVERBURN --- */}
      <div className={styles.chartRow}>
        <div className="chart-card">
          <h4><i className='bx bx-error-circle' style={{ color: 'var(--accent-red)' }}></i> Revenue Leakage</h4>
          <div className={styles.chartWrapper}>
            <Chart type="bar" width="100%" height={300} series={[ { name: 'Est Budget', data: metrics.overburn.map(p => -p.est) }, { name: 'Act Burn', data: metrics.overburn.map(p => p.act) } ]} options={{ ...chartDefaults, chart: { stacked: true }, colors: ['#a1a1aa', '#ef4444'], plotOptions: { bar: { horizontal: true, borderRadius: 0 } }, xaxis: { categories: metrics.overburn.map(p => p.name), min: -metrics.bfMax, max: metrics.bfMax, labels: { style: { colors: '#a1a1aa' }, formatter: (v) => fmtK(Math.abs(v)) } }, yaxis: { labels: { style: { colors: '#a1a1aa' }, maxWidth: 150 } }, grid: { borderColor: '#27272a', strokeDashArray: 4 }, dataLabels: { enabled: true, formatter: (v) => fmtK(Math.abs(v)) }, legend: { show: false }, tooltip: { theme: 'dark' } }} />
          </div>
        </div>
        <div className="chart-card">
        ...
        <div className="chart-card">
          ...
          <div className={styles.chartWrapper}>
            <Chart type="donut" width="100%" height={300} series={metrics.statusData.length ? metrics.statusData : [1]} options={{ ...chartDefaults, labels: metrics.statusLabels.length ? metrics.statusLabels : ['No Data'], colors: ['#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#ef4444'], stroke: { width: 0 }, plotOptions: { pie: { donut: { size: '75%' } } }, dataLabels: { enabled: false }, legend: { position: 'bottom', labels: { colors: '#a1a1aa' } } }} />
          </div>
        </div>
      </div>

      {/* --- RESTORED: ROLLOFFS & LOCATIONS --- */}
      <div className={styles.chartRowHalf}>
        <div className="chart-card">
          <h4><i className='bx bx-calendar-event' style={{ color: 'var(--text-main)' }}></i> Upcoming Availability Forecasting</h4>
      ...
      {/* --- DEEP EFFORT ANALYSIS (LOGARITHMIC STACKED) --- */}
      <div className={styles.chartRow}>
            ...
            <div style={{ width: Math.max(1200, metrics.deepEffort.labels.length * 80) + 'px' }}>
              <Chart type="bar" width="100%" height={450}
                ...
                options={{ ...chartDefaults, chart: { stacked: true, animations: { enabled: false } },
                  ...
                  // Logarithmic Y-axis ensures small data points are visible next to massive ones
                  yaxis: { logarithmic: true, labels: { style: { colors: '#a1a1aa' }, formatter: (val) => val <= 0.1 ? "0" : fmtK(val) } },
                  grid: { borderColor: '#27272a', strokeDashArray: 4 }, dataLabels: { enabled: false },
                  legend: { position: 'top', horizontalAlign: 'left', labels: { colors: '#fff' } }, tooltip: { theme: 'dark', y: { formatter: (val) => val <= 0.1 ? "0" : fmtInt(val) } }
                }} 
              />
            </div>
          </div>
