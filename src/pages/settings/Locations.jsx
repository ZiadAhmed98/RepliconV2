...
export default function Locations() {
  const { items, loading, create, update, remove } = useCrud('locations');
  ...
  return (
    <SettingsLayout title="Locations" subtitle="Office locations assigned to employees" accent="#60a5fa">
      <CrudTable columns={COLUMNS} items={items} loading={loading} onAdd={openAdd} onEdit={openEdit} onDelete={remove} />
      {modal && <ModalForm title={modal === 'add' ? 'Add Location' : 'Edit Location'} fields={FIELDS} values={form}
