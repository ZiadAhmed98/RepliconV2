...
export default function Departments() {
  const { items, loading, create, update, remove } = useCrud('departments');
  ...
  return (
    <SettingsLayout title="Departments" subtitle="Organisational departments assigned to employees" accent="#34d399">
      <CrudTable columns={COLUMNS} items={items} loading={loading} onAdd={openAdd} onEdit={openEdit} onDelete={remove} />
      {modal && <ModalForm title={modal === 'add' ? 'Add Department' : 'Edit Department'} fields={FIELDS} values={form}
